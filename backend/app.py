...
# Generate Asylum Process
@app.route('/generate_process', methods=['POST'])
def generate_process():
    nationality = request.form.get('nationality')
    age = request.form.get('age')
    gender = request.form.get('gender')
    persecution = request.form.get('persecution')
    destination = request.form.get('destination')
    minorities = request.form.getlist('minorities[]')
    minorities_text = ", ".join(minorities) if minorities else "None"

    prompt = f"Generate a comprehensive, personalized asylum process for a {nationality} individual, age {age}, gender {gender}, facing {persecution} persecution. Minority status: {minorities_text}. Destination country: {destination}. Include the following sections:\n\n"
    prompt += "A. Where to Apply: Provide exact addresses, city names, or online portals. Include contact information such as phone numbers, emails, and websites for government asylum offices.\n\n"
    prompt += "B. Next Steps in the Process: Detail what happens after applying, including interview details, preparation tips, and biometric or ID processes.\n\n"
    prompt += "C. Legal Residence: Explain when and how legal residence is granted, including duration, renewability, and types of permits (temporary, humanitarian, permanent, etc.).\n\n"
    prompt += "D. Health Insurance Access: Describe when insurance starts (immediate or delayed), emergency health options, and health card application instructions.\n\n"
    prompt += "E. Rejection and Appeal Process: Outline the steps if the asylum application is rejected, including appeal deadlines, locations, and information on legal aid or support NGOs.\n\n"
    prompt += "F. If Accepted: Welfare, Work & Integration: Provide details on access to housing, benefits, support, work eligibility timeline, and integration services like language courses and orientation.\n\n"
    prompt += "Please provide detailed and specific information for the process."

    response_text = get_openai_response(prompt)
    # Convert the response from Markdown to HTML so that when the transcript is reloaded, it displays properly.
    converted_response = markdown.markdown(response_text)

    user_id = session.get("user_id")
    if user_id:
        transcript = f"<div class='bot-message'><strong>Lugoo:</strong> {converted_response}</div>"
        chat = ChatHistory(user_id=user_id, message="Asylum process generation", response=response_text,
...
def chat_page(unique_code):
    ...
    if request.method == 'POST':
        ...
        if prompt:
            ...
            converted_answer = markdown.markdown(answer)
            conv_id = request.args.get('conversation_id')
            if conv_id:
                chat = ChatHistory.query.get(conv_id)
                # If the conversation exists, update its transcript.
                if chat:
                    new_entry = f"<div class='user-message'>{prompt}</div>"
                    new_entry += f"<div class='bot-message'><strong>Lugoo:</strong> {converted_answer}</div>"
                    chat.transcript = (chat.transcript or "") + new_entry
                    db.session.commit()
                else:
                    transcript = f"<div class='user-message'>{prompt}</div>"
                    transcript += f"<div class='bot-message'><strong>Lugoo:</strong> {converted_answer}</div>"
